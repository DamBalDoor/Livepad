...
- **Тема:** оболочка по умолчанию **тёмная**; только **светлая/тёмная** (MVP без режима system и без `prefers-color-scheme`). Переключатель на дашборде, в комнате и на login/register/preflight. Сохранение в `localStorage`, ключ **`livepad.theme`**. Monaco и панель консоли всегда на тёмном «холсте» (`--lp-editor-bg`, `--lp-console-bg`). Семантические токены интерфейса — CSS-переменные **`--lp-*`** в `apps/web/src/index.css`, утилиты Tailwind `lp-*`.
...
flowchart LR
  ...
  Browser --> Web
  Web -->|"/api и /ws → :3001"| Api
  Web -->|"/collab WS proxy → :1234"| Collab
  Api --> Db
  Collab --> Db
  Api --> Runner
```
